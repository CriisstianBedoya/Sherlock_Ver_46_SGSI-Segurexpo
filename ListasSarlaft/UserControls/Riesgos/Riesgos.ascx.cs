...
#pragma warning disable CS0414 // El campo 'Riesgos.strRecalificarRiesgos' está asignado pero su valor nunca se usa
        string strRecalificarRiesgos = "5036";
#pragma warning restore CS0414 // El campo 'Riesgos.strRecalificarRiesgos' está asignado pero su valor nunca se usa
...
#pragma warning disable CS0414 // El campo 'Riesgos.CantControlesProbabilidad' está asignado pero su valor nunca se usa
        private static int CantControlesProbabilidad = 0;
#pragma warning restore CS0414 // El campo 'Riesgos.CantControlesProbabilidad' está asignado pero su valor nunca se usa
#pragma warning disable CS0414 // El campo 'Riesgos.CantControlesImpacto' está asignado pero su valor nunca se usa
        private static int CantControlesImpacto = 0;
#pragma warning restore CS0414 // El campo 'Riesgos.CantControlesImpacto' está asignado pero su valor nunca se usa
            ...
            switch (e.CommandName)
            {
                case "Modificar":
                    ...
                    try
                    {
                        ...
                        int IdRiesgo = Convert.ToInt32(colsNoVisible[0].ToString());
...
#pragma warning restore CS0168 // La variable 'ex' se ha declarado pero nunca se usa
        ...
        private void detalleRiesgoSeleccionado()
        {
            ...
            Label96.Text = InfoGridRiesgos.Rows[RowGridRiesgos]["Nombre"].ToString().Trim();
            #endregion
        ...
        protected void imgAddActivo_Click(object sender, ImageClickEventArgs e)
        {
            //if ((Session["VerTodosProcesos"].ToString() == "0") || (Session["VerTodosProcesos"].ToString() == string.Empty))
            //{
            //    if (Session["IdProcesoRiesgo"].ToString() != Session["IdProcesoUsuario"].ToString())
            ...
            //    }
            //}
            tbActivosSel.Visible = false;
            tbListaActivos.Visible = true;
            
        }
        private void mtdLoadActivosList()
        {
            string strErrMsg = string.Empty;
            clsActivosBLL cConsulReq = new clsActivosBLL();
            List<ActivosDTO> lstEvidencias = new List<ActivosDTO>();
            cblActivos.Items.Clear();
            cConsulReq.mtdConsultarActivos(ref lstEvidencias,ref strErrMsg);
            ...
            if (lstEvidencias != null)
            {
                ...
                {
                    int iteracion = 0;
                    foreach (ActivosDTO objRegReq in lstEvidencias)
                    {
                        cblActivos.Items.Insert(iteracion, new ListItem(objRegReq.NombreActivo, objRegReq.IdActivos.ToString()));
                ...
                catch (Exception ex)
                {
                    ...
                    Mensaje("Error al cargar las premisas. " + ex.Message);
                }
            }
        }

        protected void imgRelActivos_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                List<int> lstActivos = new List<int>();
                for (int i = 0; i < cblActivos.Items.Count; i++)
                {
                    if (cblActivos.Items[i].Selected)
                    {
                        lstActivos.Add(Convert.ToInt32(cblActivos.Items[i].Value.ToString().Trim()));
                    }
                }
                int IdRiesgo = Convert.ToInt32(Session["IdRiesgo"].ToString());
                cRiesgo.mtdDeleteRiesgosvsActivos(IdRiesgo);
                foreach (int idActivo in lstActivos)
                {
                    cRiesgo.mtdAddRiesgovsActivo(IdRiesgo,idActivo,Convert.ToInt32(Session["IdUsuario"].ToString()));
                }
                tbGridActivos.Visible = true;
                tbActivosSel.Visible = true;
                mtdLoadListActivos(IdRiesgo);
                tbListaActivos.Visible = false;
            } catch (Exception ex)
            {
                Mensaje("Error! Error al adicionar los activos al riesgo: "+
                    ex.Message);
            }
            finally
            {
                Mensaje("Exito! Activos adicionados al riesgo, satisfactoriamente");
            }
        }
        private void mtdLoadListActivos(int IdRiesgo)
        {
            try
            {
                DataTable dtInfo = new DataTable();
                dtInfo = cRiesgo.GetListActivovsRiesgo(IdRiesgo);
                foreach(DataRow row in dtInfo.Rows)
                {
                    int idActivo = Convert.ToInt32( row["idActivo"].ToString());
                    for (int j = 0; j < cblActivos.Items.Count; j++)
                    {
                        if (cblActivos.Items[j].Value == idActivo.ToString())
                            cblActivos.Items[j].Selected = true;

                    }
                }
                gvListActivosvsRiesgo.DataSource = dtInfo;
                gvListActivosvsRiesgo.DataBind();
            }
            catch(Exception ex)
            {
                Mensaje("Error! Cargando la lista de activos: "+ex.Message);
            }
        }
    }
}