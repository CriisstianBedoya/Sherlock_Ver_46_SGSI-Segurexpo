...
namespace ListasSarlaft.Classes
{
    public class cCargaMasivaRCE
    {
        ...
        public void registrarRiesgo(String IdRegion, String IdPais, String IdDepartamento, String IdCiudad, String IdOficinaSucursal,
            ...
            String PerdidaEconomicaHasta, String ListaTratamiento, int IdUsuario, string fechaRegistro, string[] listActivos)
        {
            #region Variables
            ...
            try
            {
                ...
                cRiesgo dbRiesgo = new cRiesgo();
                DataTable dtRiesgo = new DataTable();
                dtRiesgo = dbRiesgo.GetLastRiesgo();
                int IdRiesgo = Convert.ToInt32(dtRiesgo.Rows[0]["UltRiesgo"].ToString());
                for(int i = 0; i < listActivos.Length; i++)
                {
                    dbRiesgo.mtdAddRiesgovsActivo(IdRiesgo, Convert.ToInt32(listActivos[i].ToString()), IdUsuario);
                }
        ...
        public byte[] mtdDescargarPlantillaRiesgos()
        {
            ...
            try
            {
                strConsulta = string.Format("SELECT [UrlArchivo],[ArchivoExcel] FROM [Riesgos].[RiesgosPlantillaMasiva] where [IdArchivos] = 7");
